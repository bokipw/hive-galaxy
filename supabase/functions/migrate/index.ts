...
Deno.serve(async (req: Request) => {
  try {
    ...
    const sql = `
      ...
      CREATE POLICY "pvp_delete_own" ON pvp_snapshots FOR DELETE USING (true);
    ...
    for (const stmt of statements) {
      const { error } = await supa.rpc('exec_sql', { query: stmt });
      if (error && !error.message.includes('already exists') && !error.message.includes('duplicate')) {
        console.error('SQL error:', error.message, 'for:', stmt.slice(0, 100));
      }
    }
